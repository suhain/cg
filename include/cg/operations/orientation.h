...
namespace cg
{
    ...
    {
        CG_CLOCKWISE = 1,
        CG_COUNTERCLOCKWISE = -1
    };

    template<class T>
    inline contour_orientation_t orientation(contour_2t<T> const& contour) 
    {
        auto it_ = std::min_element(contour.begin(), contour.end());
        auto circ = contour.circulator(it_);
        auto circ1 = circ;
        auto circ2 = circ;
        auto point = *it_;
        auto const & prev = *(--circ1);
        auto const & next = *(++circ2);
